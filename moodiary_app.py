...
def apply_custom_css():
    ...
    if is_dark:
        bg_color, main_bg, text_color, card_bg = "#121212", "rgba(40, 40, 40, 0.95)", "#f0f0f0", "#3a3a3a"
    else:
        bg_color, main_bg, text_color, card_bg = "#F8F9FA", "rgba(255, 255, 255, 1.0)", "#333333", "#ffffff"
    ...
    css = f"""
        ...
        /* 메인 컨테이너 */
        .block-container {{ 
            background: {main_bg}; border-radius: 30px; padding: 3rem !important; 
            box-shadow: 0 10px 40px rgba(0,0,0,0.05); margin-top: 2rem; max-width: 950px; 
        }}

        /* 타이틀 스타일 */
        .main-title {{ font-size: 4rem !important; font-weight: 900; color: {primary_purple}; text-align: center; letter-spacing: -2px; }}
        .animated-title {{ animation: color-shift 5s ease-in-out infinite alternate; }}
        @keyframes color-shift {{ 0% {{ color: #7B61FF; }} 100% {{ color: #FF7675; }} }}

        /* ⭐️ 사이드바 보라색 라운드 버튼 디자인 복구 */
        section[data-testid="stSidebar"] div.stButton > button {{
            ...
            margin-bottom: 10px;
            box-shadow: 0 4px 15px rgba(123, 97, 255, 0.2) !important;
        ...
        /* 달력 칸 가득 채우기 및 이모지 중앙화 */
        .fc-daygrid-day-frame {{ min-height: 110px !important; cursor: pointer; }}
        .fc-event {{ border: none !important; background: transparent !important; }}
        .fc-event-main {{ 
            display: flex !important; justify-content: center !important; 
            align-items: center !important; height: 80px !important; 
        }}
        .fc-event-title {{ font-size: 2.2em !important; text-align: center !important; }}

        /* 영화 카드: 줄거리 전체 표시 */
        .movie-card {{
            background: {card_bg}; border-radius: 15px; padding: 15px; margin-bottom: 20px;
            box-shadow: 0 4px 6px rgba(0,0,0,0.1); display: flex; gap: 15px;
        }}
        .movie-card img {{ width: 110px; border-radius: 10px; object-fit: cover; }}
        .movie-overview {{ font-size: 0.9em; line-height: 1.5; color: #555; overflow: visible; }}

        /* 행복 저장소 카드 */
        .happy-card {{ background: {card_bg}; border-left: 6px solid #FFD700; padding: 20px; border-radius: 20px; margin-bottom: 15px; box-shadow: 0 4px 10px rgba(0,0,0,0.05); }}
        .happy-date {{ font-weight: 700; font-size: 0.85em; color: #888; margin-bottom: 5px; }}
        .happy-text {{ font-size: 1.1em; font-weight: 600; color: #333; }}
...
def recommend_music(emotion):
    ...
    try:
        ...
        query = random.choice(["Daily Mix", "K-Pop Hot"])
...
def intro_page():
    ...
    st.markdown("<div class='main-title animated-title'>MOODIARY</div>", unsafe_allow_html=True)
    st.markdown("<div style='text-align:center; font-size:1.5rem; font-weight:700; color:#444; margin-bottom:3rem;'>오늘 당신의 마음은 어떤가요?</div>", unsafe_allow_html=True)
...
def login_page():
    ...
    with tab1:
        lid, lpw = st.text_input("아이디", key="l_id"), st.text_input("비밀번호", type="password", key="l_pw")
        if st.button("로그인"):
            users = get_all_users(sh)
            if lid in users and users[lid] == str(lpw):
                st.session_state.logged_in, st.session_state.username = True, lid
                st.session_state.page = "dashboard"; st.rerun()
            else: st.error("회원정보를 확인해주세요.")
    with tab2:
        nid, npw = st.text_input("새 아이디", key="n_id"), st.text_input("비밀번호(4자리)", type="password", max_chars=4, key="n_pw")
        if st.button("가입하기"):
...
def main_app():
    ...
    with st.sidebar:
        ...
        if st.button("📊 감정 통계"): st.session_state.page = "stats"; st.rerun()
...
def page_write(sh):
    ...
    txt = st.text_area("오늘 하루는 어땠나요?", height=250, placeholder="마음을 솔직하게 적어보세요.")
...
def page_recommend(sh):
    ...
    with c2:
        ...
        for item in st.session_state.get("movie_recs", []):
            st.markdown(f"""<div class="movie-card"><img src="{item['poster']}"><div class="movie-info"><b>{item['title']}</b> ({item['year']})<br><span style='color:#f1c40f;'>⭐{item['rating']}</span><p class='movie-overview'>{item['overview']}</p></div></div>""", unsafe_allow_html=True)

def page_dashboard(sh):
    st.markdown("## 📅 감정 달력")
    diaries = get_user_diaries(sh, st.session_state.username)
    events = []
    for d, data in diaries.items():
        meta = EMOTION_META.get(data['emotion'], EMOTION_META["중립"])
        # 파란색 겹침 방지를 위해 단일 이벤트를 블록형으로 설정
        events.append({
            "title": meta["emoji"], "start": d, "display": "block",
            "backgroundColor": meta["color"], "borderColor": meta["color"], "allDay": True
        })
...
def page_happy_storage(sh):
    ...
    for date, text in sorted(happy_list, reverse=True):
        st.markdown(f"<div class='happy-card'><div class='happy-date'>{date}</div><div class='happy-text'>{text}</div></div>", unsafe_allow_html=True)

...
def apply_custom_css():
    ...
    css = f"""
        ...
        /* ⭐️ 월별 구분 헤더 스타일 */
        .month-header {{
            margin: 35px 0 15px 0; padding-bottom: 8px; border-bottom: 2px solid #FFD700;
            font-size: 1.6rem; color: {main_text}; font-weight: 800;
        }}
...
def main_app():
    ...
    elif st.session_state.page == "result": st.info("분석이 완료되었습니다. 목차에서 다른 메뉴를 선택하세요.")
...
def login_page():
    ...
    with c1:
        st.markdown("<div style='padding-top: 5rem;'><h1 class='animated-title'>MOODIARY</h1><p style='font-size: 1.5rem;'>감정을 기록하고<br>나를 위한 처방을 받아보세요.</p></div>", unsafe_allow_html=True)
    with c2:
        st.markdown("<div style='padding: 2rem; border-radius: 20px; background: rgba(255,255,255,0.4);'>", unsafe_allow_html=True)

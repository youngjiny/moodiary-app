# --- 1) 필수 라이브러리 ---
import streamlit as st
import random
import requests
import torch
from transformers import AutoModelForSequenceClassification, AutoTokenizer, AutoConfig
import time
import streamlit.components.v1 as components
import json
import os
from datetime import datetime
from streamlit_calendar import calendar # ⭐️ 달력 라이브러리 추가

# --- 2) 기본 설정 ---
KOBERT_BASE_MODEL = "monologg/kobert"
KOBERT_SAVED_REPO = "Young-jin/kobert-moodiary-app"
TMDB_BASE_URL = "https://api.themoviedb.org/3"
USER_DB_FILE = "users.json"
DIARY_DB_FILE = "diary_db.json" # ⭐️ 일기 저장용 파일

# 비상용 TMDB 키
EMERGENCY_TMDB_KEY = "8587d6734fd278ecc05dcbe710c29f9c"

# ⭐️ 감정별 색상 및 이모지 설정 ⭐️
EMOTION_META = {
    "행복": {"color": "#FFD700", "emoji": "😆"}, # 노랑
    "슬픔": {"color": "#1E90FF", "emoji": "😭"}, # 파랑
    "분노": {"color": "#FF4500", "emoji": "🤬"}, # 빨강
    "힘듦": {"color": "#808080", "emoji": "🤯"}, # 회색
    "놀람": {"color": "#8A2BE2", "emoji": "😱"}, # 보라
    "중립": {"color": "#A9A9A9", "emoji": "😐"}  # (혹시 모를 기본값)
}

st.set_page_config(layout="wide", page_title="MOODIARY")

# =========================================
# 🔐 3) 데이터 관리 함수 (회원/일기)
# =========================================
def load_json(filename):
    if not os.path.exists(filename): return {}
    try:
        with open(filename, "r", encoding='utf-8') as f: return json.load(f)
    except: return {}

def save_json(filename, data):
    with open(filename, "w", encoding='utf-8') as f:
        json.dump(data, f, ensure_ascii=False, indent=4)

# 일기 저장 함수 (사용자별 -> 날짜별 저장)
def save_diary_entry(username, date, emotion, text):
    db = load_json(DIARY_DB_FILE)
    if username not in db: db[username] = {}
    db[username][date] = {"emotion": emotion, "text": text}
    save_json(DIARY_DB_FILE, db)

# 내 일기 데이터 가져오기
def get_my_diaries(username):
    db = load_json(DIARY_DB_FILE)
    return db.get(username, {})

# =========================================
# 🧠 4) AI 및 추천 로직 (기존 유지)
# =========================================
@st.cache_resource
def load_kobert_model():
    try:
        CORRECT_ID_TO_LABEL = {0: '분노', 1: '기쁨', 2: '불안', 3: '당황', 4: '슬픔', 5: '상처'}
        config = AutoConfig.from_pretrained(KOBERT_BASE_MODEL, trust_remote_code=True, num_labels=6, id2label=CORRECT_ID_TO_LABEL, label2id={l: i for i, l in CORRECT_ID_TO_LABEL.items()})
        tokenizer = AutoTokenizer.from_pretrained(KOBERT_BASE_MODEL, trust_remote_code=True)
        model = AutoModelForSequenceClassification.from_pretrained(KOBERT_SAVED_REPO, config=config, trust_remote_code=True, ignore_mismatched_sizes=False)
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        model.to(device)
        post_processing_map = getattr(model.config, 'post_processing_map', None) or {'기쁨': '행복', '슬픔': '슬픔', '상처': '슬픔', '불안': '힘듦', '당황': '놀람', '분노': '분노'}
        return model, tokenizer, device, post_processing_map
    except: return None, None, None, None

def analyze_diary_kobert(text, model, tokenizer, device, post_processing_map):
    if not text: return None, 0.0
    enc = tokenizer(text, truncation=True, padding=True, max_length=128, return_tensors="pt")
    for k in enc: enc[k] = enc[k].to(device)
    with torch.no_grad(): logits = model(**enc).logits
    probs = torch.softmax(logits, dim=1)[0]
    pred_id = int(probs.argmax().cpu().item())
    score = float(probs[pred_id].cpu().item())
    id2label = getattr(model.config, "id2label", {})
    original = id2label.get(pred_id) or id2label.get(str(pred_id)) or "중립"
    return post_processing_map.get(original, original), score

@st.cache_resource
def get_spotify_client():
    creds = st.secrets.get("spotify", {})
    cid, secret = creds.get("client_id"), creds.get("client_secret")
    if not cid or not secret: return None
    try:
        manager = SpotifyClientCredentials(client_id=cid, client_secret=secret)
        return spotipy.Spotify(client_credentials_manager=manager, retries=3, backoff_factor=0.3)
    except: return None

def recommend_music(emotion):
    sp = get_spotify_client()
    if not sp: return ["Spotify 연결 실패"]
    KR_KEYWORDS = {
        "행복": ["여행", "행복", "케이팝 최신", "여름 노래"],
        "슬픔": ["발라드 최신", "이별 노래", "감성 케이팝", "K-ballad"],
        "분노": ["인기 밴드", "팝송", "스트레스", "재즈"],
        "힘듦": ["위로 노래", "힐링 케이팝", "잔잔한 팝"],
        "놀람": ["파티 케이팝", "EDM 케이팝", "페스티벌 음악"],
    }
    query = random.choice(KR_KEYWORDS.get(emotion, ["케이팝"])) + " year:2010-2025 NOT children NOT nursery"
    try:
        res = sp.search(q=query, type="track", limit=50)
        tracks = (res.get("tracks") or {}).get("items") or []
        valid = []
        for t in tracks:
            if t.get('id') and t.get('name'):
                valid.append({"id": t['id'], "title": t['name']})
        if not valid: return ["추천 곡을 찾지 못했습니다."]
        seen = set(); unique_valid = []
        for v in valid:
            if v['id'] not in seen: unique_valid.append(v); seen.add(v['id'])
        return random.sample(unique_valid, k=min(3, len(unique_valid)))
    except Exception as e: return [f"Spotify 오류: {e}"]

def recommend_movies(emotion):
    key = st.secrets.get("tmdb", {}).get("api_key") or st.secrets.get("TMDB_API_KEY") or EMERGENCY_TMDB_KEY
    if not key: return [{"text": "TMDB 연결 실패", "poster": None}]
    GENRES = {"행복": "35|10749|10751|27", "분노": "28|12|35|878", "슬픔": "35|10751|14", "힘듦": "35|10751|14", "놀람": "35|10751|14"}
    try:
        r = requests.get(f"{TMDB_BASE_URL}/discover/movie", params={"api_key": key, "language": "ko-KR", "sort_by": "popularity.desc", "with_genres": GENRES.get(emotion), "without_genres": "16", "page": random.randint(1, 5), "vote_count.gte": 1000, "vote_average.gte": 7.5, "primary_release_date.gte": "2000-01-01"}, timeout=10)
        r.raise_for_status(); results = r.json().get("results", [])
        if not results: return [{"text": "조건에 맞는 영화가 없습니다.", "poster": None}]
        picks = random.sample(results, min(3, len(results)))
        return [{"title": m.get("title"), "year": (m.get("release_date") or "")[:4], "rating": m.get("vote_average", 0.0), "overview": m.get("overview", ""), "poster": f"https://image.tmdb.org/t/p/w500{m['poster_path']}" if m.get("poster_path") else None} for m in picks]
    except Exception as e: return [{"text": f"TMDb 오류: {e}", "poster": None}]

# =========================================
# 🖥️ 5) 화면 구성 (로그인 -> 대시보드 -> 작성)
# =========================================

# --- 페이지 1: 로그인/회원가입 ---
def login_page():
    st.title("MOODIARY 💖")
    tab1, tab2 = st.tabs(["🔑 로그인", "📝 회원가입"])
    with tab1:
        login_id = st.text_input("아이디", key="login_id")
        login_pw = st.text_input("비밀번호 (숫자 4자리)", type="password", key="login_pw")
        if st.button("로그인 하기", use_container_width=True):
            users = load_json(USER_DB_FILE)
            if login_id in users and users[login_id] == login_pw:
                st.session_state.logged_in = True
                st.session_state.username = login_id
                st.session_state.page = "dashboard" # 로그인 후 대시보드로 이동
                st.rerun()
            else: st.error("아이디 또는 비밀번호가 잘못되었습니다.")
    with tab2:
        new_id = st.text_input("새 아이디", key="new_id")
        new_pw = st.text_input("새 비밀번호 (숫자 4자리)", type="password", key="new_pw", max_chars=4)
        if st.button("가입하기", use_container_width=True):
            users = load_json(USER_DB_FILE)
            if new_id in users: st.error("이미 존재하는 아이디입니다.")
            elif len(new_id)<1 or len(new_pw)!=4 or not new_pw.isdigit(): st.error("입력 정보를 확인해주세요.")
            else:
                users[new_id] = new_pw
                save_json(USER_DB_FILE, users)
                st.success("가입 성공! 로그인해주세요.")

# --- 페이지 2: 대시보드 (달력) ---
def dashboard_page():
    st.title(f"{st.session_state.username}님의 MOODIARY 📅")
    
    # 1. 달력 데이터 준비
    my_diaries = get_my_diaries(st.session_state.username)
    calendar_events = []
    for date_str, data in my_diaries.items():
        emo = data.get("emotion", "중립")
        meta = EMOTION_META.get(emo, EMOTION_META["중립"])
        calendar_events.append({
            "title": meta["emoji"], # 달력에 이모지만 표시
            "start": date_str,
            "backgroundColor": meta["color"],
            "borderColor": meta["color"],
            "allDay": True
        })

    # 2. 달력 표시
    calendar_options = {
        "headerToolbar": {"left": "prev,next today", "center": "title", "right": ""},
        "initialView": "dayGridMonth",
    }
    calendar(events=calendar_events, options=calendar_options, custom_css="""
        .fc-event-title { font-size: 1.5em !important; text-align: center; }
        .fc-daygrid-event { cursor: default; }
    """)

    st.write("") # 여백
    # 3. 일기 쓰기 버튼
    if st.button("✏️ 오늘의 일기 쓰러 가기", type="primary", use_container_width=True):
        st.session_state.page = "write" # 작성 페이지로 이동
        st.rerun()

# --- 페이지 3: 일기 작성 및 분석 ---
def write_page(model, tokenizer, device, postmap):
    st.header("오늘의 이야기를 들려주세요 📝")
    
    # (이전으로 돌아가기 버튼)
    if st.button("⬅️ 달력으로 돌아가기"):
        st.session_state.page = "dashboard"
        st.rerun()

    diary_text = st.text_area(" ", height=200, key="current_diary", label_visibility="collapsed", placeholder="오늘 하루는 어땠나요?")

    if st.button("🔍 감정 분석하기", type="primary", use_container_width=True):
        if not diary_text.strip():
            st.warning("내용을 입력해주세요!")
            return

        with st.spinner("AI가 감정을 분석하고 있어요..."):
            # 1. 분석
            emo, sc = analyze_diary_kobert(diary_text, model, tokenizer, device, postmap)
            # 2. 저장 (오늘 날짜 기준, 덮어쓰기)
            today = datetime.now().strftime("%Y-%m-%d")
            save_diary_entry(st.session_state.username, today, emo, diary_text)
            
            # 3. 결과 세션에 저장
            st.session_state.final_emotion = emo
            st.session_state.confidence = sc
            st.session_state.music_recs = recommend_music(emo)
            st.session_state.movie_recs = recommend_movies(emo)
            
            # 4. 결과 페이지로 이동하지 않고 바로 아래에 표시 (고객 요청 흐름)

    # 분석 결과가 있으면 표시
    if st.session_state.get("final_emotion"):
        emo = st.session_state.final_emotion
        meta = EMOTION_META.get(emo, EMOTION_META["중립"])
        
        st.divider()
        st.subheader(f"오늘의 감정: {meta['emoji']} **{emo}**")
        
        col_m, col_v = st.columns(2)
        with col_m:
            st.markdown("#### 🎵 추천 음악")
            for item in st.session_state.music_recs:
                if isinstance(item, dict) and item.get('id'):
                    components.iframe(f"https://open.spotify.com/embed/track/{item['id']}?utm_source=generator", height=80)
        with col_v:
            st.markdown("#### 🎬 추천 영화")
            for item in st.session_state.movie_recs:
                if item.get('poster'):
                    ic, tc = st.columns([1, 2])
                    ic.image(item['poster'], use_container_width=True)
                    tc.markdown(f"##### {item['title']} ({item['year']})\n⭐ {item['rating']:.1f}\n\n*{item.get('overview','')[:100]}...*")

# =========================================
# 🚀 앱 메인 컨트롤러
# =========================================
if "logged_in" not in st.session_state: st.session_state.logged_in = False
if "page" not in st.session_state: st.session_state.page = "login"

# 사이드바 (로그인 상태일 때만)
if st.session_state.logged_in:
    with st.sidebar:
        st.write(f"**{st.session_state.username}**님 안녕하세요!")
        if st.button("로그아웃"):
            st.session_state.logged_in = False
            st.session_state.page = "login"
            # 세션 데이터 초기화
            for key in ["final_emotion", "music_recs", "movie_recs", "current_diary"]:
                if key in st.session_state: del st.session_state[key]
            st.rerun()

# 페이지 라우팅
if not st.session_state.logged_in:
    login_page()
elif st.session_state.page == "dashboard":
    dashboard_page()
elif st.session_state.page == "write":
    # 모델은 작성 페이지에 들어올 때만 로드 (최적화)
    model, tokenizer, device, postmap = load_kobert_model()
    if model:
        write_page(model, tokenizer, device, postmap)
    else:
        st.error("AI 모델 로드 실패. 잠시 후 다시 시도해주세요.")
